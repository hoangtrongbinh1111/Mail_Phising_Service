...
class Model(object):
    """
    This class is used to train the mail phishing detection models step by step based on NLP techniques.

    """
    def __init__(self,labId, model_type = 'lstm', train_data_dir = '', val_size = 0.1):
        """
        Init class 
        Parameters:
        -----------
        ...
        self.trainX, self.valX, self.trainY, self.valY, self.tokenizer, self.embeding_matrix = get_train_data (train_data_dir, val_size)
        # create model
        self.model = get_model(tokenizer=self.tokenizer, embedding_matrix = self.embeding_matrix, rnn_cell= model_type)
        ...
        output = open(os.path.join (self.checkpoint_dir,'embeding_matrix.pkl'), 'wb')
        pickle.dump(self.embeding_matrix, output)
        output.close()
